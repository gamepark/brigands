        ...
        if (firstPartner !== undefined) {
          if (selectedTokens === undefined) return true
            ...
            if (selectedTokens.length === maxToken - tokensAlreadyTaken) {
              ...
              return selectedTokens.length < maxToken - tokensAlreadyTaken
            }
          }
        } else {
          return false
  ...
  return (
    <div {...props} css={bankFlex}>
      {players.map((player, indexP) =>
        <div
          css={[tokenPlayerDivPosition, playerId === undefined || playerId === PlayerRole.Prince ? swapJustifyContentToStart : swapJustifyContentToStart]}
          key={indexP}>
          {getArray(player.tokens.kick).map((_, indexT) =>
            <div key={indexT}
                 css={[tokenSize, player.role === playerId && selectedTokensInBank !== undefined && selectedTokensInBank.find(tok => tok.tokenAction === TokenAction.Kicking && tok.index === indexT) && isSelected]}>
          ...
          {getArray(player.tokens.move).map((_, indexT) =>
            <div key={indexT}
                 css={[tokenSize, player.role === playerId && selectedTokensInBank !== undefined && selectedTokensInBank.find(tok => tok.tokenAction === TokenAction.Fleeing && tok.index === indexT) && isSelected]}>
          ...
          {getArray(player.tokens.steal).map((_, indexT) =>
            <div key={indexT}
                 css={[tokenSize, player.role === playerId && selectedTokensInBank !== undefined && selectedTokensInBank.find(tok => tok.tokenAction === TokenAction.Stealing && tok.index === indexT) && isSelected]}>
...
const tokenPlayerDivPosition = css`
  margin: 0.5em 1em;
  height: 100%;
  width: 10%;
  display: flex;
